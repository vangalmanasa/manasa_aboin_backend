...
const getAllSpecialRequestServiceRequestsForAllUsers = async (req, res) => {
  try {
    console.log(
      "📦 Fetching all special request service bookings for all users"
    );
    ...
    const query = `
      ...
      FROM service_requests sr
      JOIN "user" u ON sr.user_id = u.user_id
      JOIN special_request_service_bookings srsb
        ON sr.service_name = 'special_request_service'
        AND sr.service_reference_id::INT = srsb.special_request_service_id
      WHERE sr.service_name = 'special_request_service'
    `;

    const result = await pool.query(query);
    console.log("✅ Special request service bookings result:", result.rows);

    res.json({ success: true, data: result.rows });
  } catch (err) {
    console.error(
      "❌ Error in getAllSpecialRequestServiceRequestsForAllUsers:",
      err.message
    );
    res.status(500).json({ success: false, error: "Internal Server Error" });
  }
